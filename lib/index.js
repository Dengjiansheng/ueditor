    ...
    if (req.query.action === 'config') {
      ...
      res.ue_list = function (list_dir) {
        ...
        if (config.qn) {
          ...
          client.list('/', function (err, results) {
            if (err) throw err;
            var files = results.items;
            var total = files.length;
            files.forEach(function (file) {
              var filetype = 'jpg,png,gif,ico,bmp,jepg';
              var tmplist = file.key.split('.');
              var _filetype = tmplist[tmplist.length - 1];
              if (filetype.indexOf(_filetype.toLowerCase()) >= 0) {
                var temp = {};
                temp.url = 'http://static.yixinsw.org/' + file.key;
                list[i] = (temp);
              } else { }
              i++;
